...
const PARENT_FAQ = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "はい。Nobilva は野球をがんばる中高生を主な対象として設計していますが、他のスポーツや部活動に取り組まれている方も歓迎しています。無料学習面談で具体的な状況を伺ったうえで、最適なご提案をします。",
      },
      {
  ...
  },
  {
    ...
    items: [
      {
        q: "申込みの流れを教えてください",
        a: "1. 無料学習面談に申込み → 2. 24時間以内に面談日時の確定メールが届く → 3. オンラインで30分の面談 → 4. ご検討期間を経て正式申込み → 5. サービス開始。",
      },
      {
        q: "無料学習面談は本当に無料ですか？",
  ...
  },
  {
    ...
    items: [
      {
        q: "メンターはどんな人ですか？",
        a: "現在、Nobilva のメンターは代表・養田貴大と学習ヘッドコーチ・中村龍人（東京大学 計数工学科）の2名で運用しています。全生徒を直接担当しています。",
      },
      {
        q: "メンターは途中で変わりますか？",
  ...
  },
  {
    ...
    items: [
      {
        q: "野球以外の部活でも利用できますか？",
        a: "はい、ご利用いただけます。Nobilva は野球をがんばる中高生を中心にサポートしていますが、学習計画・週1面談・毎日の進捗確認という仕組みは部活動全般に対応しています。サッカー・バスケットボールなど他の部活動に取り組む生徒も受け入れており、練習や大会のスケジュールを伺ったうえで、同じように日割りの計画を作成します。",
      },
    ],
  },
];
...
const STUDENT_FAQ = [
  {
    ...
    items: [
      {
        q: "自分で勝手に申し込んでいいの？",
        a: "いいです。Nobilva は生徒本人からの申込みも受け付けています。無料学習面談のフォームで、お申し込み者欄に自分の名前を書いてください。ただし、契約・料金支払いは保護者の方を経由する形になるため、最終的にはご家族に話す必要があります。",
      },
      {
        q: "診断の面談、親がいないと話せない？",
      ...
      {
        q: "何を準備すればいい？",
        a: "特に準備は不要です。ビデオ通話ができる環境（スマホでもOK）と、30分の時間だけご用意ください。学校で配布されている教材や、今使っている問題集があれば、面談の場でメンターに見せてもらえると助かります。",
      },
    ],
  },
  {
...
const TEAM_FAQ = [
  ...
  },
  {
    ...
    items: [
      {
        q: "チーム特別価格はどれくらいですか？",
        a: "エッセンシャル 14,000円/月、ベーシック 20,000円/月です（通常価格から約22〜23%割引）。ご利用継続中ずっと有効です。チーム特別価格は保護者の月額に直接反映されるため、チーム側に金銭的な手続きは発生しません。",
      },
      {
