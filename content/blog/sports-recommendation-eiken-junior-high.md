...
- **大学進学（野球推薦・AO入試）**：英検準2級・2級のスコアが出願条件や加点に使われるケースが増えています
...
### 英検・定期テスト・大会——3つのスケジュールを重ねて管理する

この記事でお伝えしたとおり、英検は年3回、出願に間に合うタイミングは限られています。一方で、定期テストや大会の日程も動かせません。**推薦を狙うなら、英検の受験時期・定期テスト対策・大会シーズンの3つが重なる中で、何をいつ優先するかを事前に整理しておく必要があります。**

[Nobilva](/services/nobilva)は、この3つのスケジュールを重ねて「6月検定の前に定期テストがあるから、5月はテスト対策に集中→テスト後に英検ラストスパート」といった優先順位の判断を一緒に行います。何をいつ止めて、いつ再開するか——その切り替えの設計を、月18,000円〜でサポートしています。
...
:::cta-nobilva
title: 英検・評定・大会の‖スケジュール、‖一緒に整理しませんか？
description: 英検の受験時期と‖定期テスト対策を‖重ねて計画し、‖推薦に必要な準備を‖漏れなく進めます。‖月18,000円〜・‖30日全額返金保証。
